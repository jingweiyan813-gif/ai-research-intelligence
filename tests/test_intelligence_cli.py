...
def test_intelligence_dedupe_works_on_latest_items(isolated_cwd) -> None:  # type: ignore[no-untyped-def]
...
def test_intelligence_novelty_works_on_latest_items(isolated_cwd) -> None:  # type: ignore[no-untyped-def]
    state = StateStore(StoragePaths.default())
...
def test_intelligence_novelty_update_seen(isolated_cwd) -> None:  # type: ignore[no-untyped-def]
    state = StateStore(StoragePaths.default())
    item = make_item(item_id="new")
    state.save_latest_items([item.model_dump(mode="json")])
...
def test_intelligence_extract_works_on_latest_items(monkeypatch, isolated_cwd) -> None:  # type: ignore[no-untyped-def]
...
def test_intelligence_extract_no_save_does_not_write(monkeypatch, isolated_cwd) -> None:  # type: ignore[no-untyped-def]
    import airi.cli as cli_module
