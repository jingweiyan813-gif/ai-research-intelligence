...
def test_airi_trends_works_on_latest_items() -> None:
...
def test_airi_trends_update_timeseries_works() -> None:
    with runner.isolated_filesystem():
        state = StateStore(StoragePaths.default())
        state.save_latest_items(
...
def test_airi_correlate_works() -> None:
    with runner.isolated_filesystem():
        state = StateStore(StoragePaths.default())
        state.save_latest_items(
            [
...
def test_airi_correlate_apply_works(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    with runner.isolated_filesystem():
        ...
        state.save_latest_items(
            [
                item.model_copy(
                    update={"scores": scorer.score(item)}
                ).model_dump(mode="json")
            ]
        )
...
def test_airi_link_paper_repos_works() -> None:
    with runner.isolated_filesystem():
        state = StateStore(StoragePaths.default())
        state.save_latest_items(
            [
